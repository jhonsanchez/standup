...
keeps rendering (and refreshing) above it while you converse below.
`esc` interrupts a running turn (like Claude Code); `ctrl+x` closes the
dock — a still-running turn keeps working in the background and the
footer pings when the reply lands (`A` reopens the conversation). It runs your own
