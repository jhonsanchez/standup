...
`esc` or `ctrl+x` closes it — a running turn keeps working in the
background and the footer pings when the reply lands (`A` reopens the
conversation); `ctrl+c` cancels a running turn. It runs your own
